...
int req_pgorm_row_save(json *req_params, stream *response) {
...
int req_pgorm_row_delete(json *req_params, stream *response) {
	if (req_pgorm_response_sucess_begin(response)) return 1;
	char relation_schema[STR_SIZE];
	char relation_name[STR_SIZE];
	json_entry *columns_entry;
...
int req_pgorm_row_load_where(json *req_params, stream *response) {
	if (req_pgorm_response_sucess_begin(response)) return 1;
	char relation_schema[STR_SIZE];
	char relation_name[STR_SIZE];
...
int req_pgorm_row_array_load(json *req_params, stream *response) {
	if (req_pgorm_response_sucess_begin(response)) return 1;
	char relation_schema[STR_SIZE];
	char relation_name[STR_SIZE];
	char condition[32*1024];
...
int req_pgorm_row_array_delete(json *req_params, stream *response) {
	if (req_pgorm_response_sucess_begin(response)) return 1;
	char relation_schema[STR_SIZE];
	char relation_name[STR_SIZE];
	char condition[32*1024];
	...
	int rows_deleted = atoi(PQgetvalue(pg_result,0,0));
	PQclear(pg_result);
	pg_connection_unlock(pg_connection);
	if (stream_add_str(response, ",\n  \"rows_deleted\": ", NULL)) return 1;
	if (stream_add_int(response, rows_deleted)) return 1;
	if (stream_add_str(response, "\n}", NULL)) return 1;
	return 0;
}
